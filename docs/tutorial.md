...
```
┌─────────────────────────────────────────────┐
│  Your Ionic / React / Vue / Angular Web App  │
│  (TypeScript, runs inside ArkUI WebView)     │
│                                              │
│  import { Device } from '@capacitor/device' │
│  Device.getInfo() ──► window.Capacitor       │
│                           .nativePromise()   │
└────────────────────┬────────────────────────┘
                     │  JS Bridge (postMessage)
┌────────────────────▼────────────────────────┐
│  CapacitorBridge.ets  (ArkTS native layer)   │
│  Routes call ► PluginRegistry ► Plugin.ets   │
│  Plugin uses @kit.* / @ohos.* native APIs   │
│  Resolves result back to JS callback        │
└─────────────────────────────────────────────┘
```
...
| hvigorw | Command Line Tools | Build tool for OpenHarmony. Required only if not using DevEco Studio. |
...
The `--capacitor` flag sets up a `capacitor.config.ts` and installs Capacitor packages. However, the current Ionic CLI installs **Capacitor 8**, while the `@oniroproject/capacitor-openharmony` adapter requires **Capacitor 6**. You must downgrade:

```bash
# Remove the Capacitor 8 packages that ionic start installs
npm uninstall @capacitor/core @capacitor/cli @capacitor/app \
...
> **Important:** All `@capacitor/*` packages must be pinned to version 6 (`@6`) to avoid peer-dependency conflicts with the platform adapter.

If you already have an existing Ionic/React/Vue/Angular project that uses Capacitor, skip to [step 4](#4-install-the-openharmony-platform-adapter) and run the commands from that project's root.
...
> **Important:** The `appId` becomes the OpenHarmony bundle name. Use a reverse-domain format and avoid hyphens — OpenHarmony bundle IDs may only contain alphanumeric characters and dots.
...
Install both layers together. Pin the `@capacitor/*` packages to **version 6** to match the platform adapter:
...
> **Why two packages?** Your web code imports from `@capacitor/device` etc. — these are the official Capacitor packages and ship the TypeScript API. The `@oniroproject/*` packages contain the `.ets` native code that `cap sync` copies into the `openharmony/` native project. Both must be installed as `dependencies`.
...
> **Note:** `@capacitor/preferences` must be installed alongside `@oniroproject/capacitor-preferences` (see [step 6](#6-install-plugin-packages)).
...
> **Tip:** Writing to `Directory.Documents` on OpenHarmony triggers the system `DocumentViewPicker`, allowing the user to choose where to save the file.
...
> **Note:** You must use the full npm package name `@oniroproject/capacitor-openharmony` — not just `openharmony`. The Capacitor CLI resolves third-party platforms by their package name.
...
> **Note:** Without signing, `hdc install` will fail with a signature verification error.
...
> **Tip:** Replace `com.example.myoniroapp` with the `appId` you set in `capacitor.config.ts`.
...
- **Explore the demo app** — the [`ionic-openharmony-demo/`](../ionic-openharmony-demo/) directory contains a complete Ionic React app that demonstrates all seven plugins with real code.
- **Browse plugin source** — each plugin's ArkTS implementation lives under [`plugins/`](../plugins/). Reading these is the best way to understand the native API mapping and extend or debug plugin behaviour.
- **Architecture deep-dive** — [`docs/phase4_status.md`](./phase4_status.md) documents the bridge design, plugin registry pattern, and ArkTS compatibility decisions in detail.
...
- **Oniro Project** — visit [oniroproject.org](https://oniroproject.org) for broader ecosystem resources, device targets, and community links.
